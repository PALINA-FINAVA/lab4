...
// Элемент по умолчанию включен (отмечен флажком)
       ...
       /* ModulFunc = graphicsMenu.add(new JCheckBoxMenuItem("Показать модуль f"));
        ModulFunc.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (ModulFunc.isSelected()) {
                    String valueX = "0";
                    String valueY = "0";
                    display.setXDigits(Integer.parseInt(valueX));
                    display.setYDigits(Integer.parseInt(valueY));
                    display.setShowGrid(ModulFunc.isSelected());
...
// Доступность или недоступность элементов меню "График" определяется загруженностью данных
            ...
            //ModulFunc.setEnabled(fileLoaded);
